...
useAudioPlayer는 N=1/묶음 끝 경계 당김 + 묶음 내부 `gapSkipTarget` 점프 + **일반 재생(비반복) 경로의 `gapSkipTarget` 점프**(2026-07 추가 — 그전엔 건너뛰기가 전부 `isGlobalLoopActiveRef` 안에만 있어 **반복을 켜야만** 동작했다). 세 경로 모두 같은 순수 함수를 쓰므로 기준이 어긋날 수 없다. **speechEnd 없는 문장은 문장별로 기존 동작 폴백. `speechOnlyRef`는 loopNRef와 같은 이유로 sync-effect deps 금지.**
