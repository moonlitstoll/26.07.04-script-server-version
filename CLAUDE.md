...
- **엔진**: `utils/speechSegments.js` 순수 함수가 유일한 경계 계산원 — **간격 3초(GAP_SKIP_MIN) 초과일 때만 건너뜀**(겹치는 대사·짧은 정적은 그대로 이어 재생 = 병합 로직 불필요), 끝쪽 0.8s 패딩(SPEECH_TAIL_PAD). **`SPEECH_TAIL_PAD`가 곧 '대사 잘림 안전 여유'다** — 건너뛰기는 정확히 `대사끝 + PAD`에서 발동하므로 모델이 그보다 이르게 답한 만큼 대사가 잘린다. `GAP_SKIP_MIN`(3초)은 *어디를* 건너뛸지 고르는 기준일 뿐 잘림 여유가 아니다(혼동 주의). 0.4 → 0.8로 올림(2026-07): 실측 잘림은 없었지만 여유가 0.4초뿐이라 모델이 1초만 일찍 답해도 잘렸다. 대가는 구간당 건너뛰는 시간 0.4초 감소뿐이고, **건너뛰기 성립 여부는 안 바뀐다**(점프 조건은 `gap > PAD + bufferTime + 0.05`인데 gap>3 필수 + bufferTime 최대 2.0이라 전 범위 성립 — 전수 확인). useAudioPlayer는 N=1/묶음 끝 경계 당김 + 묶음 내부 `gapSkipTarget` 점프 + **일반 재생(비반복) 경로의 `gapSkipTarget` 점프**(2026-07 추가 — 그전엔 건너뛰기가 전부 `isGlobalLoopActiveRef` 안에만 있어 **반복을 켜야만** 동작했다). 세 경로 모두 같은 순수 함수를 쓰므로 기준이 어긋날 수 없다. **speechEnd 없는 문장은 문장별로 기존 동작 폴백. `speechOnlyRef`는 loopNRef와 같은 이유로 sync-effect deps 금지.**
